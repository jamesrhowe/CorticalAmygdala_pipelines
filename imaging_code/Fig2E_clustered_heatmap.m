...
%%
% now we can plot the dendrogram and heatmap together:
panelE = figure('units','normalized','outerposition',[0 0 0.95 0.5]);
% first, we show the dendrogram on the rightmost subplot:
subplot(1,size(normF3,3)+2,size(normF3,3)+2)
H = dendrogram(tree,0,'reorder', leafOrder,...
    'Orientation','left','ColorThreshold',cthresh);

set(H,'LineWidth',0.8); 
set(gca,'xtick',[],'ytick',1,'yticklabel',size(normF3,1),'box','off', ...
    'tickdir','out','FontName','Arial','FontSize',10);

% then we iterate through the number of odors we have
for o=1:size(normF3,3)
    subplot(1,size(normF3,3)+2,o+1)
    imagesc(flipud(normF3(leafOrder,:,o)))

    clim([options.cmin options.cmax]); colormap viridis
    xline(50.5,'r','LineWidth',1.5) %annotate the odor period
    xline(75.5,'r','LineWidth',1.5)
    
    if o == 1
        set(gca,'xtick',[],'ytick',[size(normF3,1)],'tickdir','out','FontSize',20)
    else        
    ...
    boundaries = find(diff(flipud(T(leafOrder)))~=0);
    for b = 1:length(boundaries)
        yline(boundaries(b)+1.5,'w','LineWidth',1.5);
    end
    set(gca,'box','off','tickdir','out','LineWidth',0.5,...
        'XColor','none','YColor','none','FontName','Arial')
    title(options.odnames(o),'FontSize',12,'FontName','Arial')
end

% leftmost subplot will have the colorbar
subplot(1,size(normF3,3)+2,1)
cbar=colorbar; clim([options.cmin options.cmax])
cbar.TickDirection='out'; cbar.Ticks=options.cmin:2:options.cmax;
cbar.TickLength =0.01;  cbar.LineWidth =2; cbar.Box ='off';

x1=get(gca,'position'); x=get(cbar,'Position');
x(3)=0.01; set(cbar,'Position',x); 
set(gca,'FontSize',20,'FontName','Arial')
%%
f6 = figure('Position',[0,0,400,700]);
...
%%
saveas(f6,'E:\Paper Figures\20240407_paper_figures\final_fig\G_avgplot',...
    'png')
saveas(f6,'E:\Paper Figures\20240407_paper_figures\final_fig\G_avgplot',...
    'svg')