...
params:
  id: "plCoA"
  class: "Neuron"
  path: "output/sequencing/04-subcluster_datasets-RNA/"
  dimn: 15
...
glut_clusters <- c(0:6, 11:13, 16)
gaba_clusters <- c(7:10, 14:15)

class_clusters[class_clusters %in% glut_clusters] <- "Glut"
class_clusters[class_clusters %in% gaba_clusters] <- "GABA"

# replace clusters with curated types, reorder for plotting
array <- AddMetaData(array, as.factor(class_clusters), col.name = "Class")
array$Class <- factor(array$Class, levels = c("Glut", "GABA"))
Idents(array) <- "Class"
...
src_list <- mapply(function(tab_name, dr_type) {
    src <- c(
        ...
        "cluster_plot_formatted(array, '{{dr_type}}', 'Class', neuron_class_colors)",  
        "```",
        ""
    )
    knit_expand(text = src)
...
```{r 17-class_relative_batch, message = FALSE, warning = FALSE, results = "hide", fig.height = 4, fig.width = 3}
relative_bar_plot_formatted(plot_group_array, 'Class', 'Batch', batch_colors_rna)
```

#### Region

```{r 18-class_relative_region, message = FALSE, warning = FALSE, results = "hide", fig.height = 4, fig.width = 3}
...
absolute_bar_plot_formatted(plot_group_array, 'Region', 'Class', neuron_class_colors)
```
...
relative_bar_plot_formatted(plot_group_array, 'Region', 'Class', neuron_class_colors) +
...
absolute_bar_plot_formatted(plot_group_array, 'Batch', 'Class', neuron_class_colors)
```
...
relative_bar_plot_formatted(plot_group_array, 'Batch', 'Class', neuron_class_colors) +
  geom_hline(yintercept = table(array$Class)[2] / length(array$Class), linetype = "dashed")
```
...
src_list <- mapply(function(metric, label) {
    src <- c(
        ...
        "violin_metrics_formatted(array, 'Class', '{{metric}}', '{{label}}', neuron_class_colors)",  
        "```",
        ""
    )
    knit_expand(text = src)
...
```{r 27-dot_tg, message = FALSE, warning = FALSE, results = "hide", fig.height = 2, fig.width = 12}
array <- PrepSCTFindMarkers(array)
...
```{r 28-stacked_vln_marker, message = FALSE, warning = FALSE, results = "hide", fig.height = 12, fig.width = 12}
...
```{r 29-heatmap_marker, message = FALSE, warning = FALSE, results = "hide", dev = "png", fig.height = 8, fig.width = 16}
DoHeatmap(array, features = marker_features, group.by = 'Class', size = 0,
          group.colors = neuron_class_colors) +
          scale_fill_gradientn(colours = heatmap_colors) +  
          scale_colour_manual(values = neuron_class_colors)
```
...
```{r 30-region_dendrogram, message = FALSE, warning = FALSE, results = "hide"}
...
```{r 31-library_dendrogram, message = FALSE, warning = FALSE, results = "hide"}
...
```{r 32-class_dendrogram_region, message = FALSE, warning = FALSE, results = "hide"}
# create region+class convolution ID
array <- AddMetaData(array, metadata = paste(array$Region, array$Class), col.name = "Region_Class")

class_region_dend <- create_dendrogram(array, as.factor(array$Region_Class), region_neuron_class_colors)
...
```{r 33-class_dendrogram_batch, message = FALSE, warning = FALSE, results = "hide"}
# create region+class convolution ID
array <- AddMetaData(array, metadata = paste(array$orig.ident, array$Class), col.name = "Batch_Class")

class_batch_dend <- create_dendrogram(array, as.factor(array$Batch_Class), batch_neuron_class_colors_rna)

plot(class_batch_dend[[2]], axes = FALSE)
```
...
#### Class with Batch ID {.tabset}

##### Distance

```{r 40-cluster_heatmap_region_dist, message = FALSE, warning = FALSE, results = "hide", fig.height = 30, fig.width = 30}
dendrogram_heatmap(class_batch_dend, method_id = "distance")
```

##### Correlation

```{r 41-cluster_heatmap_region_cor, message = FALSE, warning = FALSE, results = "hide", fig.height = 30, fig.width = 30}
dendrogram_heatmap(class_batch_dend, method_id = "correlation")
```
...
```{r 43-pseudobulk_pca_region-class, message = FALSE, warning = FALSE, results = "hide"}
pseudobulk_pca(array, "Region_Class", "Region_Class", region_neuron_class_colors)
```

#### Batch-Class

```{r 44-pseudobulk_pca_batch-class, message = FALSE, warning = FALSE, results = "hide"}
pseudobulk_pca(array, "Batch_Class", "Batch_Class", batch_neuron_class_colors_rna)
```
...
for (i in 1:length(levels(array$Class))){
  ...
  saveRDS(subset_array, file = paste0(path, id, "_", levels(array$Class)[i], "_rd1_RNA.rds"))
  gc()
}

saveRDS(array, file = paste0(path, id, "_neuron_class_rd1_RNA.rds"))
gc()
...
write.csv(cluster_file, file = "output/sequencing/cluster_lists/02D-Neuron_class_clusters_rd1.csv", col.names = TRUE)
```
