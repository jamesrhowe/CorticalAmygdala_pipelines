...
params:
  id: "plCoA"
  class: "Immune"
title: "Transform and visualize subclustered datasets (final): `r params$id` Immune"
...
array[[1]] <- readRDS("output/sequencing/03-processed_datasets-RNA/plCoA_Micro_reprocessed_RNA.rds")
array[[2]] <- readRDS("output/sequencing/03-processed_datasets-RNA/plCoA_Macro_reprocessed_RNA.rds")
...
# replace types
array <- AddMetaData(array, array$Class, col.name = "Type")
array <- AddMetaData(array, array$Class, col.name = "Type.Marker")

array$Type[array$Type == "Macro"] <- macro_clusters$Type
array$Type.Marker[array$Type.Marker == "Macro"] <- macro_clusters$Type.Marker

array$Type <- as.factor(array$Type)
array$Type <- factor(array$Type, levels = c("Micro", "Macro.1", "Macro.2"))
...
plot_merge_array <- cbind.data.frame(array$nCount_RNA, 
                               ...
                               array$Region,
                               array$Type)
colnames(plot_merge_array) <- c("UMIs", "Genes", "Batch", "Region", "Type")
...
```{r 28-dr_consolidated, message = FALSE, warning = FALSE, results = "hide"}
src_list <- mapply(function(tab_name, dr_type) {
    src <- c(
        "## {{tab_name}} {.unnumbered}",
        "```{r type-{{tab_name}}, message = FALSE, warning = FALSE}",
        "cluster_plot_formatted(array, '{{dr_type}}', 'Type', immune_colors)",  
        "```",
        ""
    )
    knit_expand(text = src)
...
```{r 29-type_absolute, message = FALSE, warning = FALSE, results = "hide"}
src_list <- mapply(function(br, br_colors) {
    src <- c(
        "### {{br}} {.unnumbered}",
...
```{r 30-type_relative_batch, message = FALSE, warning = FALSE, results = "hide", fig.height = 4, fig.width = 3}
relative_bar_plot_formatted(plot_group_array, 'Type', 'Batch', batch_colors_rna)
```

#### Region

```{r 31-type_relative_region, message = FALSE, warning = FALSE, results = "hide", fig.height = 4, fig.width = 3}
...
```{r 32-chi_square_type, message = FALSE, warning = FALSE}
...
```{r 33-propeller_type, message = FALSE, warning = FALSE}
...
```{r 34-region_relative, message = FALSE, warning = FALSE, results = "hide", fig.height = 4, fig.width = 3}
absolute_bar_plot_formatted(plot_group_array, 'Region', 'Type', immune_colors)
```

#### Relative

```{r 35-region_relative, message = FALSE, warning = FALSE, results = "hide", fig.height = 4, fig.width = 3}
relative_bar_plot_formatted(plot_group_array, 'Region', 'Type', immune_colors)
```

#### Chi-square results

```{r 36-chi_square_region, message = FALSE, warning = FALSE}
...
```{r 37-region_relative, message = FALSE, warning = FALSE, results = "hide", fig.height = 4, fig.width = 4}
absolute_bar_plot_formatted(plot_group_array, 'Batch', 'Type', immune_colors)
```

#### Relative

```{r 38-region_relative, message = FALSE, warning = FALSE, results = "hide", fig.height = 4, fig.width = 4}
relative_bar_plot_formatted(plot_group_array, 'Batch', 'Type', immune_colors) 
```

#### Chi-square results

```{r 39-chi_square_region, message = FALSE, warning = FALSE}
...
```{r 40-vln_consolidated_type, message = FALSE, warning = FALSE, results = "hide"}
src_list <- mapply(function(metric, label) {
    src <- c(
        "### {{label}} {.unnumbered}",
        "```{r vln-type-{{metric}}, message = FALSE, warning = FALSE}",
        "violin_metrics_formatted(array, 'Type', '{{metric}}', '{{label}}', immune_colors)",  
        "```",
        ""
    )
    knit_expand(text = src)
...
```{r 41-dot_tg, message = FALSE, warning = FALSE, results = "hide", fig.height = 2, fig.width = 12}
markers_list <- c("Tmem119", "Ptprc",
                  "Nav2", "Hexb",
                  "F13a1", "Itsn1",
                  "Cd74", "Ccr2")
...
```{r 42-stacked_vln_marker, message = FALSE, warning = FALSE, results = "hide", fig.height = 12, fig.width = 12}
...
```{r 43-heatmap_marker, message = FALSE, warning = FALSE, results = "hide", dev = "png", fig.height = 8, fig.width = 16}
DoHeatmap(array, features = markers_list, group.by = 'Type', size = 0,
          group.colors = immune_colors) +
          scale_fill_gradientn(colours = heatmap_colors) +  
          scale_colour_manual(values = immune_colors)
```
