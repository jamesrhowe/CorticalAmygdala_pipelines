...
params:
  ...
  output_path: "output/sequencing/03-processed_datasets-RNA/"
...
remove_barcodes <- read.csv("output/sequencing/cluster_lists/02G-remove_glut.csv", row.names = 1)

# exclude barcodes filtered in step 2
array <- array[, !colnames(array) %in% remove_barcodes$ID]
...
```{r 3-total_nuclei, message = FALSE, warning = FALSE, fig.height = 4, fig.width = 3}
...
```{r 5-batch_violin, message = FALSE, warning = FALSE, results = "hide", fig.height = 4, fig.width = 4}
src_list <- mapply(function(metric, label){
    src <- c(
        "#### {{label}} {.unnumbered}",
        "```{r batch_violin-{{metric}}, message = FALSE, warning = FALSE, fig.height = 4, fig.width = 4}",
...
```{r 6-region_violin, message = FALSE, warning = FALSE, results = "hide", fig.height = 4, fig.width = 4}
src_list <- mapply(function(metric, label){
    src <- c(
        "#### {{label}} {.unnumbered}",
        "```{r region_violin-{{metric}}, message = FALSE, warning = FALSE, fig.height = 4, fig.width = 4}",
...
src_list <- lapply(c(1:2, pc_info$Optimal_PCs), function(pcs) {
    src <- c(
        ...
        "```{r pca-loadings-{{pcs}}, message = FALSE, warning = FALSE, dev = 'png'}",
...
saveRDS(array, file = paste0(output, id, "_merged_filtered_RNA.rds"))
```
