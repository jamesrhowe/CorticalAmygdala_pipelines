...
params:
  id: "plCoA"
  input_path: "output/sequencing/01-preprocess-RNA/"
  output_path: "output/sequencing/02-merge-RNA/"
  dimn: 30
title: "Merge, transform, and format RNA datasets: `r params$id`"
date: "`r format(Sys.time(), '%d %B, %Y')`"
---

# Introduction

This notebook merges the individual libraries into a single dataset, adds the appropriate metadata for each, transforms/normalizes the data using SCTransform, and then performs linear dimensionality reduction initially via principal component analysis, followed by non-linear dimensionality reduction via tSNE and UMAP (see methods for references). 
...
for (i in 1:length(paths)){
  ...
  paths[i] <- gsub("output/sequencing/01-preprocess-RNA/","", paths[i])
  paths[i] <- gsub("_RNA_preprocess.rds","", paths[i])
...
```{r 3-total_nuclei, message = FALSE, warning = FALSE}
...
```{r 15-elbow, message = FALSE, warning = FALSE}
ElbowPlot(array, ndims = 50)
```
...
array <- RunTSNE(array, verbose = FALSE, dims.use = 1:dimn)
array <- RunUMAP(array, verbose = FALSE, dims = 1:dimn, n.epochs = 1000, min.dist = 0.5)
```
...
src_list <- mapply(function(tab_name, dr_type, dim1, dim2) {
    src <- c(
        ...
        "cluster_plot_formatted(array, '{{dr_type}}', 'orig.ident', batch_colors)",  
        "```",
        ""
    )
    knit_expand(text = src)
...
```{r 21-dr_region, message = FALSE, warning = FALSE,  results = "hide"}
src_list <- mapply(function(tab_name, dr_type, dim1, dim2) {
    src <- c(
        "### {{tab_name}} {.unnumbered}",
        "```{r region-{{tab_name}}, message = FALSE, warning = FALSE}",
...
```{r 22-dr_umis, message = FALSE, warning = FALSE, results = "hide"}
...
```{r 23-dr_genes, message = FALSE, warning = FALSE, results = "hide"}
src_list <- mapply(function(tab_name, dr_type) {
    src <- c(
        "### {{tab_name}} {.unnumbered}",
...
```{r 24-dr_mito, message = FALSE, warning = FALSE, results = "hide"}
src_list <- mapply(function(tab_name, dr_type) {
    src <- c(
        "### {{tab_name}} {.unnumbered}",
...
```{r 25-dr_ribo, message = FALSE, warning = FALSE, results = "hide"}
src_list <- mapply(function(tab_name, dr_type) {
    src <- c(
        "### {{tab_name}} {.unnumbered}",
...
```{r 26-save_output, message = FALSE, warning = FALSE}
saveRDS(array, file = paste0(output, id, "_merged.rds"))
```
